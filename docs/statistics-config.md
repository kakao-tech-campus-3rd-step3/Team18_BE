...
statistics:
  ...
  admission-year:
    ...
    min-year: 1990
  disclosure:
    # 마감 직전 이 시간 동안은 지원 현황(지원자 수 + 분포)을 공개하지 않는다.
    blackout-before-deadline: PT5M
  precompute:
    enabled: true
...
- **마감 직전 5분은 지원 현황을 공개하지 않는다.** 마감이 임박한 시점의 지원자 수는 "지금 넣어도 승산이
  없다"는 신호로 읽혀 지원 포기를 유발하는데, 그 구간에는 판단을 뒤집을 시간도 없다. `totalApplicants`만
  숨기면 버킷 count의 합으로 총원이 그대로 드러나므로 분포까지 함께 감춘다. **마감 후에는 다시 공개하므로
  정보가 영구히 사라지지는 않는다.** 이 구간의 응답은 `totalApplicants`가 생략되고 `results`가 빈 배열이며
  `notice`에 사유가 담긴다.
