...
public record ClubReviewResponseDto(
                @Schema(description = "후기 목록") List<Review> reviews) {
        public record Review(
                        @Schema(description = "후기 id", example = "1") Long id,
                        @Schema(description = "익명 작성자", example = "익명1") String writer,
                        @Schema(description = "후기 내용", example = "최고의 개발 동아리입니다.") String content,
                        @Schema(description = "작성일", example = "2024-07-23T10:00:00") LocalDateTime createdAt) {
        }
